...
| `get_board_info`            | Read-only. Full catalog record for a single board id (MCU, clock, RAM/ROM, frameworks, vendor, debug tools). Selects the exact id; lists near-miss ids as alternatives.                                                                |
| `init_project`              | Scaffolds a PlatformIO project for a board, or non-destructively adds an environment to an existing one. Skips dependency install by default (installs lazily on first build).                                                         |
| `generate_compile_commands` | Writes `compile_commands.json` (`pio run -t compiledb`) for editor/clangd IntelliSense. Cold projects install the toolchain first (needs network).                                                                                     |
...
The `v0.1` execution layer plus project-scaffolding and IDE-integration tools (14 total). Deferred to later releases: library management (search/install/list) and MCP Apps inline UI for the monitor panel.
